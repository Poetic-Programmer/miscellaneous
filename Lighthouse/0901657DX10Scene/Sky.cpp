...
Sky::Sky()
: md3dDevice(0), mVB(0), mIB(0), mCubeMapRV(0)
{
	mNumIndices = 0;
}
...
{
	md3dDevice = device;
	mCubeMapRV   = GetTextureMgr().createTex(L"Textures/grassenvmap1024.dds");
...
void Sky::draw()
{
	...
	SkyMap::fxWVPVar->SetMatrix((float*)WVP);
	SkyMap::fxCubeMapVar->SetResource(mCubeMapRV);
	...
	SkyMap::Tech->GetDesc( &techDesc );

    for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		ID3D10EffectPass* pass = SkyMap::Tech->GetPassByIndex(p);
