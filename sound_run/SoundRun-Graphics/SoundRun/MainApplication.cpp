...
bool SoundRunApp::Init()
{
	...
	Effects::InitializeAll(md3dDevice);
	InputLayouts::InitializeAll(md3dDevice);
	RenderStates::InitializeAll(md3dDevice);

	m_game.Initialize(md3dDevice);
...
void SoundRunApp::OnResize()
{
	...
	m_game.OnResize(0.25f * MathHelper::Pi, AspectRatio(), 1.0f, 1000.0f);
}
...
void SoundRunApp::Render()
{
	...
	case GameScreen:	
		...
		md3dImmediateContext->RSSetState(m_currentRS);

		m_game.Render(md3dImmediateContext);
