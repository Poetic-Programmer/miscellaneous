/*
	...
	Description:
	Implementation of the Wave class that represents a sampled sound in PCM wave format.
	See Wave.hpp for further information on the design.
...
Wave::Wave (const std::wstring &aFilename, unsigned short aChannels, unsigned short aBitDepth, unsigned long aSampleRate, size_t aSize, char *aData)
{
	mFileName = aFilename;
...
Wave::Wave (const std::wstring &aFilename)
{
	mFileName = aFilename;
	memset ((void*)&mWaveFormat, 0, sizeof (WaveFormat));
	mSize = 0;
	mWaveData = NULL;
	mStatus = Undefined;
...
unsigned int Wave::GetNumberOfSamples () const
{
	if (mSize == 0) {
		return 0;
	} else {
...
unsigned int Wave::GetSampleLengthInMilliseconds () const 
{
	if (mSize == 0) {
		return 0;
	} else {
...
//=== Saves the Wave object to a file in .wav format; returns true if write operation successful.
//=== If the file already exists then it is overwritten.

// use mFileName as the file name.
bool Wave::SaveToFile () 
{
...
bool Wave::SaveToFileAs (const std::wstring &aFilename) {
	mFileName = aFilename;
	return SaveToFile ();
} // end SaveToFileAs function.

//=== Returns status as a descriptive string.
std::wstring& Wave::GetStatusDescription () const 
{
	static std::wstring descriptions[] = {
	L"OK",
	L"Undefined",
	L"Buffer allocation (malloc) error",
	L"File open error",
	L"End of file",
	L"Not a RIFF/WAVE file",
	L"Chunk not found",
	L"Error writing RIFF header",
	L"fmt chunk not found",
	L"Invalid fmt chunk",
	L"Not in PCM format",
	L"Invalid fmt chunk size (should be 16 bytes)",
	L"Error writing fmt chunk",
	L"No data chunk",
	L"Error reading data chunk",
	L"Error writing data chunk"
};
...
void Wave::ReadWaveFile (ifstream *anInfile)
{
	...
	// read in format fields into WaveFormat struct.
	anInfile->read ((char*)&mWaveFormat, sizeof(WaveFormat));
	if (!anInfile->good()) {
		memset ((void*)&mWaveFormat, 0, sizeof (WaveFormat));
		mStatus = InvalidFmt;
		return;
...
bool Wave::FindChunk (ifstream *anInfile, unsigned long aTag)
{
	unsigned long inTag;
...
void Wave::WriteWaveFile (ofstream *anOutfile)
{
	...
	anOutfile->write ((char*)(&mWaveFormat), sizeof(WaveFormat));
	if (!anOutfile->good()) {
		mStatus = FmtWriteError;
		return;
	}
